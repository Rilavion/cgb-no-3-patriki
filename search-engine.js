window.CGB_SEARCH=(function(){
  const CACHE_KEY="cgb-search-cache";
  const TTL=5*60*1000;
  let cache=null;
    ...
    if(auth&&auth.state&&auth.state.client){
      ...
      await Promise.all([
        ...
        (async()=>{try{
          ...
          if(rows) rows.forEach(r=>data.push({group:"Обучение",title:r.title,hint:(r.excerpt||"").slice(0,90),href:"learn.html#l="+r.id,kw:(r.title+" "+(r.excerpt||"")+" "+(r.content||"")).toLowerCase().slice(0,400)}));
        }catch(e){}})(),
        (async()=>{try{
